...
class function_objects:
    ...
    def __init__(self, listadoTuplas=[(), (), ()]):
        ...
        listadoInstancias = [Object(tupla[0], tupla[1])
                             for tupla in self.listadoTuplas]

        self.listadoInstancias = sorted(listadoInstancias,
                                        key=operator.attrgetter('ref_dist'))

        intervalo = range(len(self.listadoInstancias))

        for index in intervalo:
            self.listadoInstancias[index].index = index
            self.listadoInstancias[index].index_previo = ((index - 1)
                                                          if index > 0
                                                          else "")
            self.listadoInstancias[index].index_sgte = ((index + 1)
    ...
    def neighbors(self, Object, epsi):
        ...
        # "HACIA ATRAS"
        # Aqui se arman las condiciones
        if Object.index_previo != "":
            previo = self.listadoInstancias[Object.index_previo]
            cond_1 = previo.ref_dist != ""
            cond_2 = (Object.ref_dist - previo.ref_dist) <= epsi
            # y aqui comienza el recorrido
            while cond_1 and cond_2:
                distancia = func_distance([previo.latitud,
                                           previo.longitud],
                                          [Object.latitud,
                                           Object.longitud])
                if distancia <= epsi:
                    # Se le agrega un atributo a la instancia
                    previo.distancia = distancia
                    # Se agrega la instancia evaluada.
                    # TODAVIA NO ESTAN ORDENADOS
                    vecinosDesordenados.append(previo)

                if previo.index_previo != "":
                    previo = self.listadoInstancias[previo.index_previo]
                    cond_1 = previo.ref_dist != ""
                    cond_2 = (Object.ref_dist - previo.ref_dist) <= epsi
                else:
                    break
        ...
        # "HACIA ADELANTE"
        # Aqui se arman las condiciones
        if Object.index_sgte != "":
            sgte = self.listadoInstancias[Object.index_sgte]
            cond_1 = sgte.ref_dist != ""
            cond_2 = (sgte.ref_dist - Object.ref_dist) <= epsi
            # y aqui comienza el recorrido
            while cond_1 and cond_2:
                distancia = func_distance([sgte.latitud,
                                           sgte.longitud],
                                          [Object.latitud,
                                           Object.longitud])
                if distancia <= epsi:
                    # Se le agrega un atributo a la instancia
                    sgte.distancia = distancia
                    # Se agrega la instancia evaluada.
                    # TODAVIA NO ESTAN ORDENADOS
                    vecinosDesordenados.append(sgte)

                if sgte.index_sgte != "":
                    sgte = self.listadoInstancias[sgte.index_sgte]
                    cond_1 = sgte.ref_dist != ""
                    cond_2 = (sgte.ref_dist - Object.ref_dist) <= epsi
                else:
                    break
...
class Object:
    ...
    def __init__(self, latitud, longitud,
        ...
        self.index_previo = ""
        self.index_sgte = ""
        self.ref_dist = np.sqrt(np.square(latitud) + np.square(longitud))
...
def ti_optics(SetOfObjects, epsi, MinPts, metadata=None):
...
if __name__ == "__main__":
    ...
    resultado = ti_optics(conjunto_de_puntos, 2, 2)
